...
public class JPtyImpl implements JPtyInterface {
    ...
    public int forkpty(int[] amaster, byte[] name, jtermios.Termios term, WinSize win) {
        ...
        if (ioctl(fdSlave, I_PUSH, "ptem") < 0 || // pseudo-terminal hardware emulation module,
            ioctl(fdSlave, I_PUSH, "ldterm") < 0 || // standard terminal line discipline, and
            ioctl(fdSlave, I_PUSH, "ttcompat") < 0) { // not sure but both xterm and DtTerm do it.
            close(fdSlave);
            close(fdMaster);
            return -1;
        }
